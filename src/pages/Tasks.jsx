...
const Tasks = () => {
  ...
  const pendingTask = tasks?.tasks?.filter(task => task.status === "pending")
  const runningTask = tasks?.tasks?.filter(task => task.status === "running")
  const doneTask = tasks?.tasks?.filter(task => task.status === "done")
        ...
        <div className="grid grid-cols-3 gap-5 mt-10">
          <div className="relative h-[800px] overflow-auto">
            ...
            <div className="space-y-3">
              {/* <TaskCard /> */}
              {
                pendingTask?.map(task => task.status === 'pending' && <TaskCard
                  key={task.id}
                  task={task}
                ></TaskCard>)
              }
          ...
          <div className="relative h-[800px] overflow-auto">
            ...
            <div className="space-y-3">
              ...
              {/* <TaskCard /> */}
              {
                runningTask?.map(task => task.status === 'running' && <TaskCard
                  key={task.id}
                  task={task}
                ></TaskCard>)
              }
          ...
          <div className="relative h-[800px] overflow-auto">
            ...
            <div className="space-y-3">
              {/* <TaskCard /> */}
              {
                doneTask?.map(task => task.status === 'done' && <TaskCard
                  key={task.id}
                  task={task}
                ></TaskCard>)
              }
